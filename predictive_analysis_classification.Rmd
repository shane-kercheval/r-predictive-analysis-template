...
```{r}
random_forest <- randomForest(target ~ ., data = classification_train, ntree = 2000)
predictions_raw <- predict(random_forest, classification_test %>% select(-target), type = "prob")
observations <- classification_test$target

df_temp <- data.frame(target = classification_train$target, predictions = predictions)

df_temp$no_prob <- predictions_raw[, "no"]
df_temp$yes_prob <- predictions_raw[, "yes"]
df_temp$predictions_class <- predict(random_forest, classification_test %>% select(-target))
calData1 <- calibration(target ~ no_prob + yes_prob, data = df_temp, cuts = 10)
xyplot(calData1, auto.key = list(columns = 2))


df_temp$Label <- ifelse(observations == "no", 
                              "True Outcome: No", 
                              "True Outcome: Yes")

### Plot the probability of bad credit
histogram(~no_prob|Label,
          data = df_temp,
          layout = c(2, 1),
          nint = 20,
          xlab = "Probability of No",
          type = "count")

histogram(~yes_prob|Label,
          data = df_temp,
          layout = c(2, 1),
          nint = 20,
...
confusionMatrix(data = df_temp$predictions_class, reference = df_temp$target)

### ROC curves:
library(ROCR)
test_probabilities <- predict(random_forest, newdata = classification_test, type = 'prob')
test_probabilities_target <- test_probabilities[, 'yes']


roc_pred <- prediction(predictions = test_probabilities_target, labels = classification_test$target)
...
```{r substages_analysis_logistic_auc, echo=FALSE}
# calculate AUC
perf_auc <- performance(roc_pred, measure = 'auc')
logistic_auc <- unlist(perf_auc@y.values)
```
...
library(pROC)
creditROC <- roc(relevel(df_temp$target, "yes"), df_temp$no_prob)
