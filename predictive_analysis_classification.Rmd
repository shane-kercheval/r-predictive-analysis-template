...
if(class_imbalance <= 0.3) {
	cat(paste0('\n\n> Class imbalance, min class proportion is `', percent(class_imbalance),'`. Consider up-sampling/down-sampling techniques.\n\n'))
}
``` 
...
train_classification <- function(	model_name,
	...
	############
	#so, for each fold/repitition in the *original* cross validation, use the 'test' set to calculate the ideal threshold
	...
	roc_thresholds <- map_dbl(training_control$index, ~ {
		...
		model_roc <- roc(actual_observations, predicted_probabilities_positive, levels = rev(c(target_positive_class, target_negative_class)))
...
model_plots <- function(model, testing_set, mean_roc_threshold){
	...
	predicted_class_mean_roc <- factor(ifelse(df_results$predicted_probabilities > mean_roc_threshold,
											 target_positive_class, target_negative_class),
									  levels = c(target_positive_class, target_negative_class))

	confusion_matrix_mean_roc <- table(actual = actual_observations, predictions = predicted_class_mean_roc)
	confusion_ls_mean_roc <- confusion_list(true_pos = confusion_matrix_mean_roc[target_positive_class, target_positive_class],
									true_neg = confusion_matrix_mean_roc[target_negative_class, target_negative_class],
									false_pos = confusion_matrix_mean_roc[target_negative_class, target_positive_class],
									false_neg = confusion_matrix_mean_roc[target_positive_class, target_negative_class])
	quality_mean_roc <- quality_of_model(confusion_ls_mean_roc)
	
	cat('\n\n#### Quality, cross-validated (training set) cutoff\n\n')
	print(visualize_quality_of_model(	confusion_ls_mean_roc,
	...
	return (c(	model_auc = model_auc,
				...
				training_cv_threshold = mean_roc_threshold,
				training_cv_sensitivity = quality_mean_roc$sensitivity,
				training_cv_specificity = quality_mean_roc$specificity))
}
```
...
final_models_metrics <- data.frame(	model = factor(	as.character(top_models$model),
									...
									training_cv__threshold = map_dbl(model_stats, ~ .['training_cv_threshold']),
									training_cv__sensitivity = map_dbl(model_stats, ~ .['training_cv_sensitivity']),
									training_cv__specificity = map_dbl(model_stats, ~ .['training_cv_specificity']))
...
final_models_metrics_long <- final_models_metrics %>%
	...
	dplyr::mutate(type = factor(type, levels = c('cross_validation', 'test_set', 'top_left', 'youden', 'training_cv'))) %>%
