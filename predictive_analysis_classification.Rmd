...
numeric_models <- list(
		glm_no_pre_processing = glm_no_pre_processing,
		glm_basic_processing = glm_basic_processing
		)
...
```{r top_models, echo=FALSE, fig.height=6, fig.width=9}
...
model_plots <- function(model, testing_set, prediction_threshold = 0.35){
	...
	# set.seed(123)
	# model <- randomForest(target ~ ., data = classification_train, ntree = 2000)
	#library(C50)
	#set.seed(123)
	#model <- C5.0(classification_train %>% select(-target), classification_train$target) # exclude 'default' column, but include it as target factor vector for classification
	# summary(model)

	probabilities_raw <- predict(model, testing_set %>% dplyr::select(-target), type = 'prob')
	...
	print(histogram(~probabilities_negative | label, data = df_results, layout = c(2, 1), nint = 20, xlab = paste0('Probability of `', target_negative_class,'`'), type = 'count'))
...
# 	require(klaR)
# 	training_predictions <- predict(model, classification_train %>% dplyr::select(-target), type = 'prob')
# 	recalibration_training <- data.frame(actual_observations = model$trainingData$.outcome, predicted_probabilities = training_predictions[[target_positive_class]])
# 	recalibrated_model <- NaiveBayes(actual_observations ~ predicted_probabilities, data = recalibration_training, usekernel = TRUE)
# 	
# #	new_predictions <- factor(ifelse(recalibration_training$predicted_probabilities >= 0.5, target_positive_class, target_negative_class), levels = c(target_positive_class, target_negative_class))
# #	confusionMatrix(data = new_predictions, reference = recalibration_training$actual_observations)
# 
...
# 	print(recal_chart)
# 
# 	recal_roc_pred <- prediction(predictions = recalibrated_probabilities, labels = actual_observations)
# 	recal_roc_perf <- performance(recal_roc_pred, measure = 'sens', x.measure = 'fpr')
# 	print(plot(recal_roc_perf, colorize = TRUE, print.cutoffs.at = seq(0,1,0.05), text.adj = c(-0.2, 1.7)))
# 	print(plot(roc_perf, colorize = TRUE, print.cutoffs.at = seq(0,1,0.05), text.adj = c(-0.2, 1.7)))
# 	# calculate AUC
# 	recal_perf_auc <- performance(recal_roc_pred, measure = 'auc')
# 	recal_model_auc <- unlist(recal_perf_auc@y.values)
# 	recal_model_auc
	
	# pick predictions with threshold based off of AUC
	new_predictions <- factor(ifelse(recalibrated_probabilities >= prediction_threshold, target_positive_class, target_negative_class), levels = c(target_positive_class, target_negative_class))
	conf_matrix <- confusionMatrix(data = new_predictions, reference = df_results$actual_observations)
	cat(codebc(conf_matrix))
