...
```{r classification_no_pre_processing, echo = FALSE}
if(refresh_models)
{
...
```{r classification_basic_processing, echo = FALSE}
if(refresh_models)
{
	check_data(classification_train, sample_to_predictor_ratio_threshold = 5)
	set.seed(custom_seed)
...
```{r logistic_regression_collinear_caret, echo = FALSE}
if(refresh_models)
{
	check_data(classification_train[, recommended_columns_caret], sample_to_predictor_ratio_threshold = 5)
	set.seed(custom_seed)
...
```{r logistic_regression_collinear_calc, echo = FALSE}
if(refresh_models)
{
	check_data(classification_train[, recommended_columns_custom], sample_to_predictor_ratio_threshold = 5)
	set.seed(custom_seed)
...
```{r linear_discriminant_analsysis, echo = FALSE}
if(refresh_models)
{
	check_data(classification_train, sample_to_predictor_ratio_threshold = 5)
	set.seed(custom_seed)
...
```{r linear_discriminant_analsysis_remove_collinear, echo = FALSE}
if(refresh_models)
{
	check_data(classification_train[, recommended_columns_caret], sample_to_predictor_ratio_threshold = 5)
	set.seed(custom_seed)
...
```{r partial_least_squares_discriminant_analysis, echo = FALSE}
if(refresh_models)
{
	check_data(classification_train, sample_to_predictor_ratio_threshold = 5)
	set.seed(custom_seed)
...
```{r glmnet_lasso_ridge, echo = FALSE}
if(refresh_models)
{
	check_data(classification_train, sample_to_predictor_ratio_threshold = 5)
	set.seed(custom_seed)
...
```{r regularized_discriminant_analysis, echo = FALSE}
if(refresh_models)
{
	check_data(dataset = classification_train, class_to_predictor_ratio_threshold = 5, class_names = c(target_positive_class, target_negative_class))
	set.seed(custom_seed)
	regularized_discriminant_analysis <- train(	target ~ .,
											data = classification_train,
											method = 'rda',
											metric=metric,
											preProc=c('nzv', 'center', 'scale'),
											trControl = train_control,
											tuneGrid = expand.grid(	lambda = tuning_rda_lambda,
																	gamma = tuning_rda_gamma))
	saveRDS(regularized_discriminant_analysis, file = './classification_data/regularized_discriminant_analysis.RDS')
} else{
	regularized_discriminant_analysis <- readRDS('./classification_data/regularized_discriminant_analysis.RDS')
}
#regularized_discriminant_analysis
summary(regularized_discriminant_analysis)
predictors(regularized_discriminant_analysis)
plot(regularized_discriminant_analysis, top = 20, scales = list(y = list(cex = 0.95)))
varImp(regularized_discriminant_analysis, scale = FALSE)
```

### Regularized Discriminant Analysis - Removed Correlated Predictors

```{r regularized_discriminant_analysis_rc, echo = FALSE}
if(refresh_models)
{
	check_data(dataset = classification_train, class_to_predictor_ratio_threshold = 5, class_names = c(target_positive_class, target_negative_class))
	set.seed(custom_seed)
	regularized_discriminant_analysis_rc <- train(	target ~ .,
											data = classification_train[, recommended_columns_caret],
											method = 'rda',
											metric=metric,
											preProc=c('nzv', 'center', 'scale'),
											trControl = train_control,
											tuneGrid = expand.grid(	lambda = tuning_rda_lambda,
																	gamma = tuning_rda_gamma))
	saveRDS(regularized_discriminant_analysis_rc, file = './classification_data/regularized_discriminant_analysis_rc.RDS')
} else{
	regularized_discriminant_analysis_rc <- readRDS('./classification_data/regularized_discriminant_analysis_rc.RDS')
}
#regularized_discriminant_analysis_rc
summary(regularized_discriminant_analysis_rc)
predictors(regularized_discriminant_analysis_rc)
plot(regularized_discriminant_analysis_rc, top = 20, scales = list(y = list(cex = 0.95)))
varImp(regularized_discriminant_analysis_rc, scale = FALSE)
```

### Mixture Discriminant Analysis

```{r mixture_discriminant_analysis, echo = FALSE}
if(refresh_models)
{
	check_data(dataset = classification_train, class_to_predictor_ratio_threshold = 5, class_names = c(target_positive_class, target_negative_class))
	set.seed(custom_seed)
	mixture_discriminant_analysis <- train(	target ~ .,
											data = classification_train,
											method = 'mda',
											metric=metric,
											preProc=c('nzv', 'center', 'scale'),
											trControl = train_control,
											tuneGrid = expand.grid(subclasses = tuning_mda_subclasses))

	saveRDS(mixture_discriminant_analysis, file = './classification_data/mixture_discriminant_analysis.RDS')
} else{
	mixture_discriminant_analysis <- readRDS('./classification_data/mixture_discriminant_analysis.RDS')
}
#mixture_discriminant_analysis
summary(mixture_discriminant_analysis)
predictors(mixture_discriminant_analysis)
plot(mixture_discriminant_analysis, top = 20, scales = list(y = list(cex = 0.95)))
varImp(mixture_discriminant_analysis, scale = FALSE)
```

### Mixture Discriminant Analysis - Removed Correlated Predictors

```{r mixture_discriminant_analysis_rc, echo = FALSE}
if(refresh_models)
{
	check_data(dataset = classification_train, class_to_predictor_ratio_threshold = 5, class_names = c(target_positive_class, target_negative_class))
	set.seed(custom_seed)
	mixture_discriminant_analysis_rc <- train(	target ~ .,
											data = classification_train[, recommended_columns_caret],
											method = 'mda',
											metric=metric,
											preProc=c('nzv', 'center', 'scale'),
	...
	saveRDS(mixture_discriminant_analysis_rc, file = './classification_data/mixture_discriminant_analysis_rc.RDS')
} else{
	mixture_discriminant_analysis_rc <- readRDS('./classification_data/mixture_discriminant_analysis_rc.RDS')
}
#mixture_discriminant_analysis_rc
summary(mixture_discriminant_analysis_rc)
predictors(mixture_discriminant_analysis_rc)
plot(mixture_discriminant_analysis_rc, top = 20, scales = list(y = list(cex = 0.95)))
varImp(mixture_discriminant_analysis_rc, scale = FALSE)
```
...
classification_models <- list(
		...
		regularized_discriminant_analysis = regularized_discriminant_analysis,
		regularized_discriminant_analysis_rc = regularized_discriminant_analysis_rc,
		mixture_discriminant_analysis = mixture_discriminant_analysis,
		mixture_discriminant_analysis_rc = mixture_discriminant_analysis_rc
		)
