...
glm_no_pre_process <- train_classification(
	...
	dataset = classification_train[, classification_column_names],
	training_method = 'glm',
...
```{r neural_network_spatial_rc, eval=FALSE, include=FALSE, comment='', results='asis'}
max_size <- max(tuning_nnet_size)
training_numWts <- (max_size * ncol(classification_train[, recommended_columns_caret])) + max_size + 1 # from pg 361, but slightly different because their `reducedSet` (vector rather than df) does not include the target variable, so ignoring the `+1` they have
...
neural_network_spatial_rc <- train_classification(
	...
	class_to_predictor_ratio_threshold = 5,
	class_names = c(target_positive_class, target_negative_class),
	neural_net_numWts = training_numWts,
	neural_net_lineout = parameter_nnet_linout,
	neural_net_max_iterations = 2000)
...
```{r neural_network_spatial_rc_skew, eval=FALSE, include=FALSE, comment='', results='asis'}
max_size <- max(tuning_nnet_size)
training_numWts <- (max_size * ncol(classification_train[, recommended_columns_caret])) + max_size + 1 # from pg 361, but slightly different because their `reducedSet` (vector rather than df) does not include the target variable, so ignoring the `+1` they have
...
model_stats <- pmap(list(as.character(top_models$model), top_models$model_object, top_models$training_preProcess), function(.x, .y, .z){
	...
	if(refresh_models)
	{
		...
		if(model_method == 'asdf')
		{
			...
			set.seed(custom_seed)
			final_model <- train(model_formula, data = classification_train, method = model_method, metric = metric, preProc = pre_process, trControl = final_train_control, tuneGrid = tune_grid)
		}
