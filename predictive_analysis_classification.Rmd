...
```{r logistic_regression_stepwise_backward, echo = FALSE}
stepwise_logistic_columns <- NULL
if(refresh_models) {
	check_data(classification_train, sample_to_predictor_ratio_threshold = 5)
...
train_classification <- function(	model_name,
	...
	if(refresh_models) {
		...
		training_max_iterations <- NULL
		training_numWts <- NULL
		training_lineout <- NULL
		if(training_method == 'nnet') {
			
			training_max_iterations <- 2000
			training_lineout <- parameter_nnet_linout

			max_size <- max(training_tune_grid$size)
			training_numWts <- (max_size * ncol(dataset)) + max_size + 1 # from pg 361, but slightly different because their `reducedSet` (vector rather than df) does not include the target variable, so ignoring the `+1` they have

			cat(paste0('\n\nSetting `nnet` specific parameters: `MaxNWts` - `', training_numWts,'`; `maxit` - `', training_max_iterations,'`\n\n'))
		...
		caret_model <- train(	target ~ .,
												...
												tuneGrid = training_tune_grid,
							  					...
							  					maxit = training_max_iterations,
							  					MaxNWts = training_numWts,
							  					linout = training_lineout
							  					)
	...
	if(!is.null(training_tune_grid)) { # if there are tuning parameters, we can plot them.
		
		cat(paste('\n\n####', model_name), '\n\n')
		print(plot(caret_model, top = 20, scales = list(y = list(cex = 0.95))))
		print(plot.new())
		cat('\n\n')
		cat(caret_model$method)
		cat('\n\n')
	}
...
spot_check_models <- tribble(
	~training_method, ~model_name, ~training_dataset_columns, ~training_preProcess, ~training_tune_grid, ~training_tune_length, ~sample_to_predictor_ratio_threshold, ~class_to_predictor_ratio_threshold, ~class_names, ~function_post_train, ~trained_model,
	'glm', 'glm_no_pre_process', classification_column_names, NULL, NULL, NULL, 5, NULL, NULL, NULL, NULL,
	'glm', 'glm_basic_processing', classification_column_names, preProc=c('nzv', 'center', 'scale'), NULL, NULL, 5, NULL, NULL, NULL, NULL,
	'glm', 'glm_remove_collinearity_caret', recommended_columns_caret, preProc=c('nzv', 'center', 'scale'), NULL, NULL, 5, NULL, NULL, NULL, NULL,
	'glm', 'glm_remove_collinearity_custom', recommended_columns_custom, preProc=c('nzv', 'center', 'scale'), NULL, NULL, 5, NULL, NULL, NULL, NULL,
	'glm', 'logistic_regression_stepwise_backward', stepwise_logistic_columns, preProc=c('nzv', 'center', 'scale'), NULL, NULL, 5, NULL, NULL, NULL, NULL,
	'lda', 'linear_discriminant_analsysis', classification_column_names, preProc=c('nzv', 'center', 'scale'), NULL, NULL, 5, NULL, NULL, NULL, NULL,
	'lda', 'linear_discriminant_analsysis_remove_collinear', recommended_columns_caret, preProc=c('nzv', 'center', 'scale'), NULL, NULL, 5, NULL, NULL, NULL, NULL,
	'pls', 'partial_least_squares_discriminant_analysis', classification_column_names, preProc=c('nzv', 'center', 'scale'), expand.grid(ncomp = tuning_number_of_latent_variables_to_retain), NULL, 5, NULL, NULL, NULL, NULL,
	'glmnet', 'glmnet_lasso_ridge', classification_column_names, preProc=c('nzv', 'center', 'scale'), expand.grid(alpha = tuning_glmnet_alpha, lambda = tuning_glmnet_lambda), NULL, 5, NULL, NULL, NULL, NULL,
	'sparseLDA', 'sparse_lda', classification_column_names, preProc=c('nzv', 'center', 'scale'), NULL, 5, 5, NULL, NULL, NULL, NULL,
	#'pam', 'nearest_shrunken_centroids', classification_column_names, preProc=c('nzv', 'center', 'scale'), tuning_nearest_shrunken_centroids_shrinkage_threshold, NULL, 5, NULL, NULL, NULL, NULL,
	'rda', 'regularized_discriminant_analysis', classification_column_names, preProc=c('nzv', 'center', 'scale'), expand.grid(lambda = tuning_rda_lambda, gamma = tuning_rda_gamma), NULL, NULL, 5, c(target_positive_class, target_negative_class), NULL, NULL,
	'rda', 'regularized_discriminant_analysis_rc', recommended_columns_caret, preProc=c('nzv', 'center', 'scale'), expand.grid(lambda = tuning_rda_lambda, gamma = tuning_rda_gamma), NULL, NULL, 5, c(target_positive_class, target_negative_class), NULL, NULL,
	'mda', 'mixture_discriminant_analysis', classification_column_names, preProc=c('nzv', 'center', 'scale'), expand.grid(subclasses = tuning_mda_subclasses), NULL, NULL, 5, c(target_positive_class, target_negative_class), NULL, NULL,
	'mda', 'mixture_discriminant_analysis_rc', recommended_columns_caret, preProc=c('nzv', 'center', 'scale'), expand.grid(subclasses = tuning_mda_subclasses), NULL, NULL, 5, c(target_positive_class, target_negative_class), NULL, NULL,
	'nnet', 'neural_network_spatial_rc', recommended_columns_caret, preProc=c('nzv', 'center', 'scale', 'spatialSign'), expand.grid(size = tuning_nnet_size, decay = tuning_nnet_decay), NULL, NULL, 5, c(target_positive_class, target_negative_class), NULL, NULL
	)

stopifnot(!duplicated(spot_check_models$model_name))

walk(1:nrow(spot_check_models), ~ {
	spot_check_model <- spot_check_models[., ]
	
	cat(paste('###', spot_check_model$model_name), '\n\n')
	
	trained_model <- train_classification(	model_name = spot_check_model$model_name,
						dataset = classification_train[, spot_check_model$training_dataset_columns[[1]]],
						custom_seed = custom_seed,
						training_method = spot_check_model$training_method,
						training_metric = metric,
						training_preProcess = spot_check_model$training_preProcess[[1]],
						training_control = train_control,
						training_tune_grid = spot_check_model$training_tune_grid[[1]],
						training_tune_length = spot_check_model$training_tune_length[[1]],
						refresh_models = refresh_models,
						sample_to_predictor_ratio_threshold = spot_check_model$sample_to_predictor_ratio_threshold[[1]],
						class_to_predictor_ratio_threshold = spot_check_model$class_to_predictor_ratio_threshold[[1]],
						class_names = spot_check_model$class_names[[1]])

	spot_check_models[., ]$trained_model[[1]] <<- trained_model
	if(!is.null(spot_check_model$function_post_train[[1]])) {
		spot_check_model$function_post_train[[1]]()
	}
})
```
...
```{r neural_network, echo = FALSE}

#neural_network <- train_classification()
...
for(index in 1:nrow(spot_check_models)) {
	
	classification_models <- c(classification_models, spot_check_models[index,]$trained_model)
}
names(classification_models) <- map_chr(1:nrow(spot_check_models), ~ spot_check_models[., ]$model_name)
...
top_models <- inner_join(top_models, spot_check_models %>% dplyr::select(model_name, training_preProcess) %>% rename(model = model_name), by = c('model'))
